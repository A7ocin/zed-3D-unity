...
{
    public GameObject zed, zedPivot;

    public override void MarkerDetectedSingle(Vector3 worldposition, Quaternion worldrotation)
    {
        gameObject.SetActive(true); 

        zed.transform.position = -worldposition;
        //transform.rotation = worldrotation;
        zedPivot.transform.rotation = Quaternion.Inverse(worldrotation);
        Vector3 alignPlane = new Vector3(-90, 0, 0);
        transform.Rotate(alignPlane, Space.World);
        zedPivot.transform.Rotate(alignPlane, Space.World);
    }
